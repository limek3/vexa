      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr 1fr', gap: 20, alignItems: 'flex-start' }}>
        ...
        <div className="col">
          {section === 'theme' && <>
            ...
            <Card>
              <div className="section-title" style={{ marginBottom: 14 }}>Accent color</div>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {[
                  { v: 'clay',   label: 'Глина',   color: '#C57854' },
                  { v: 'sage',   label: 'Шалфей',  color: '#7AA08A' },
                  { v: 'indigo', label: 'Индиго',  color: '#5E6FCB' },
                  { v: 'plum',   label: 'Слива',   color: '#A04F84' },
                  { v: 'amber',  label: 'Янтарь',  color: '#D8A04A' },
                ].map(c => (
                  <button key={c.v} onClick={() => setTweak('accent', c.v)} style={{
                    ...
                    <span style={{ width: 18, height: 18, borderRadius: 999, background: c.color, boxShadow: tweaks.accent === c.v ? '0 0 0 3px color-mix(in oklab, ' + c.color + ' 25%, transparent)' : 'none' }} />
          ...
          {section === 'page' && <>
              ...
              <div className="grid-3">
                {[
                  ...
                  <button key={c.v} onClick={() => setCoverStyle(c.v)} style={{
                    ...
                    <div style={{
                      height: 80,
                      background: c.v === 'gradient' ? 'linear-gradient(135deg, color-mix(in oklab, var(--accent) 30%, transparent), color-mix(in oklab, oklch(0.65 0.10 220) 25%, var(--surface-3)))' :
            ...
            <Card>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <ToggleRow label="Watermark «Powered by КликБук»" sub="Доступно в платных тарифах" />
              </div>
            </Card>
          </>}
          ...
          {section === 'typography' && (
            <Card>
              ...
              <div className="grid-2">
                {[
                  { v: 'Manrope',  label: 'Manrope · стандарт' },
                  { v: 'Inter',    label: 'Inter' },
                ].map(f => (
                  <button key={f.v} style={{
                    padding: 16, textAlign: 'left',
                    border: '1.5px solid ' + (f.v === 'Manrope' ? 'var(--accent)' : 'var(--line)'),
                    background: f.v === 'Manrope' ? 'var(--accent-soft)' : 'var(--surface)',
                    borderRadius: 'var(--r-sm)', cursor: 'pointer', font: 'inherit',
                  }}>
                    <div style={{ fontFamily: f.v, fontSize: 22, fontWeight: 600, letterSpacing: '-0.01em' }}>Aa</div>
...
function ThemeCard({ item, active, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{
        height: 100,
        background: isDark ? '#14130F' : isSys ? 'linear-gradient(90deg, #F7F5F0 50%, #14130F 50%)' : '#F7F5F0',
        position: 'relative',
        borderBottom: '1px solid var(--line)',
      }}>
        <div style={{ position: 'absolute', left: 12, top: 12, width: 8, height: 8, borderRadius: 999, background: isDark ? '#3A3830' : '#D4CDB9' }} />
        <div style={{ position: 'absolute', left: 12, top: 28, width: 60, height: 5, borderRadius: 999, background: isDark ? '#3A3830' : '#D4CDB9' }} />
        <div style={{ position: 'absolute', left: 12, top: 40, width: 40, height: 5, borderRadius: 999, background: isDark ? '#3A3830' : '#D4CDB9', opacity: 0.5 }} />
