...
export function ChatsPage() {
  ...
  useEffect(() => {
    if (active && unread[active.id]) setUnread({ ...unread, [active.id]: 0 });
  }, [activeId]);
  ...
  const sendMessage = (text) => {
    ...
    const newMsg = {
      id: `m${Date.now()}`, from: 'me', text, time, read: false,
  ...
  const sendSpecial = (msg) => {
    ...
    const now = new Date();
    const time = `${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
    setStore({ ...store, [active.id]: [...activeMsgs, { id: `m${Date.now()}`, from: 'me', time, read: false, ...msg }] });
    setTimeout(scrollToBottom, 30);
  };
  ...
  useEffect(() => {
    if (draft.startsWith('/')) setTmplOpen(true);
    else if (tmplOpen && !draft.startsWith('/')) setTmplOpen(false);
  }, [draft]);

  // click-outside to close popovers
  useEffect(() => {
    const h = (e) => {
      if (!e.target.closest('.composer') && !e.target.closest('.attach-menu') && !e.target.closest('.emoji-panel')) {
        setAttachOpen(false);
        setEmojiOpen(false);
      }
    };
  ...
  return (
    <div data-screen-label="04 Chats" className="page-fixed">
      <div className="page-head">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Btn kind="secondary" icon="sparkle">Шаблоны</Btn>
          <Btn kind="secondary" icon="filter">Фильтры</Btn>
          <Btn kind="primary" icon="edit">Написать клиенту</Btn>
        </div>
      </div>
          ...
          {active ? (
            <div className="chat-col conv">
              ...
              <div className="conv-toolbar">
                ...
                <Btn size="sm" kind="ghost" icon="calendar">Предложить время</Btn>
                <Btn size="sm" kind="ghost" icon="services">Прислать услугу</Btn>
                <Btn size="sm" kind="ghost" icon="card">Запросить оплату</Btn>
                <div className="spacer" />
                <Btn size="sm" kind="ghost" icon="sparkle">Создать ответ</Btn>
              </div>
              ...
              <div className="conv-foot" style={{ position: 'relative' }}>
                ...
                {!recording && (
                  <div style={{ display: 'flex', gap: 4, marginBottom: 8, overflow: 'auto', paddingBottom: 1 }}>
                    {QUICK_REPLIES.map((r, i) => (
                      <button key={i} className="chip sm" style={{ flexShrink: 0 }} onClick={() => sendMessage(r)}>{r}</button>
                    ))}
                  </div>
                )}

                <div className={`composer ${recording ? 'recording' : ''}`} style={{ position: 'relative' }}>
                  {recording ? (
                    <>
                    ...
                    <>
                      <button className="btn btn-ghost icon" data-tip="Прикрепить"
                        onClick={() => { setAttachOpen(o => !o); setEmojiOpen(false); }}>
                        <Icon name="paperclip" size={14} style={{ color: attachOpen ? 'var(--accent)' : undefined }} />
                      </button>
                      <textarea
                        ref={composerRef}
                        value={draft}
                        ...
                        rows={1}
                      />
                      <button className="btn btn-ghost icon" data-tip="Эмодзи"
                        onClick={() => { setEmojiOpen(o => !o); setAttachOpen(false); }}>
                        <span style={{ fontSize: 15, lineHeight: 1 }}>{emojiOpen ? '😊' : '🙂'}</span>
                      </button>
                      <button className="btn btn-ghost icon" data-tip="Шаблоны (/)"
                        onClick={() => { setTmplOpen(o => !o); composerRef.current?.focus(); }}>
                        <Icon name="sparkle" size={14} style={{ color: tmplOpen ? 'var(--accent)' : undefined }} />
                      </button>
                  ...
                  {emojiOpen && !recording && (
                    <div className="emoji-panel">
                      {['❤️','👍','🌿','🙏','🔥','👏','😊','✨','😍','😅','🥹','💛','🤍','🤝','💯','☕️','🌸','🌺','🎉','🌷','💆‍♀️','💇‍♀️','✂️','💄'].map(e => (
                        <button key={e} onClick={() => { setDraft(d => d + e); setEmojiOpen(false); composerRef.current?.focus(); }}>{e}</button>
                      ))}
                    </div>
                  )}
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 6, fontSize: 11, color: 'var(--text-4)' }}>
                  <span>↵ — отправить · ⇧↵ — перенос · «/» — шаблоны</span>
                  <span>{draft.length > 0 && `${draft.length} симв.`}</span>
                </div>
              </div>
            </div>
          ) : (
...
/* ----------- Message ----------- */
function MessageBlock({ m, prev, grouped, last, onReact, onReply, onPin, isPinned, clientName, allMsgs }) {
  ...
  // booking renders its own container (not bubble)
  if (m.type === 'booking') {
    return (
