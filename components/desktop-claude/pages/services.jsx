...
export function ServicesPage() {
  ...
  const toggleField = (id, field) => {
    setServices(s => s.map(x => x.id === id ? { ...x, [field]: !x[field] } : x));
  };
    ...
    <div data-screen-label="05 Services">
      <div className="page-head">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Btn icon="copy" kind="secondary">Дублировать</Btn>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: editing ? '1fr 380px' : '1fr', gap: 16, alignItems: 'flex-start' }}>
        ...
        {editing && <ServiceEditor s={editing} onClose={() => setEditing(null)} />}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="card hoverable" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
        ...
        <button className="btn btn-ghost icon" onClick={e => { e.stopPropagation(); }}><Icon name="more-v" size={13} /></button>
      </div>
...
function ServiceEditor({ s, onClose }) {
  ...
  return (
    <Card style={{ position: 'sticky', top: 'calc(var(--topbar-h) + 24px)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <Btn kind="primary" style={{ flex: 1 }}>{s.id ? 'Сохранить' : 'Создать'}</Btn>
