...
/* Shared icons + UI primitives for КликБук */
...
export const Icon = ({ name, size = 16, className = '', style }) => {
  ...
  switch (name) {
    ...
    case 'refresh': return <svg {...common}><path d="M21 12a9 9 0 0 1-15.3 6.36L3 16"/><path d="M3 21v-5h5"/><path d="M3 12a9 9 0 0 1 15.3-6.36L21 8"/><path d="M21 3v5h-5"/></svg>;
    ...
    case 'smile': return <svg {...common}><circle cx="12" cy="12" r="9"/><path d="M8.5 10h.01M15.5 10h.01"/><path d="M8.8 14.2c1.55 1.45 4.85 1.45 6.4 0"/></svg>;
...
export const NumberPopIn = ({ value = '0', className = '' }) => {
  ...
  return (
    <span key={playKey} className={`t-digit-group is-animating ${className}`} aria-label={text}>
      {text.split('').map((ch, index) => (
        <span
          key={`${ch}-${index}`}
          className="t-digit"
          style={{ '--digit-index': Math.min(index, 8) }}
          aria-hidden="true"
        >
          {ch === ' ' ? '\u00A0' : ch}
        </span>
...
export const SuccessCheck = ({ active = true, size = 14, className = '' }) => (
  <span className={`t-success-check ${className}`} data-state={active ? 'in' : 'out'} aria-hidden="true">
...
function readableActionLabel(children, icon) {
  if (typeof children === 'string') return children;
  if (Array.isArray(children)) {
    const text = children.filter((item) => typeof item === 'string').join(' ').trim();
    if (text) return text;
  }
...
export const Btn = ({ children, kind = 'secondary', size, icon, onClick, className = '', type = 'button', ...rest }) => {
  ...
  const handleClick = (event) => {
    ...
    if (rest['data-record-action'] && typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('clickbook-desktop-action', {
        detail: { label: actionLabel },
      }));
    }
  };

  return (
    <button type={type} onClick={handleClick} className={`btn btn-${kind} ${size ? size : ''} ${className}`} data-desktop-ui-button="true" {...rest}>
...
export const Switch = ({ on, onChange, label }) => (
  <button
    ...
    role="switch"
    aria-checked={!!on}
    aria-label={label || (on ? 'Выключить' : 'Включить')}
    onClick={() => onChange?.(!on)}
  />
);
...
export const Check = ({ on, onChange, label }) => (
  <button
    ...
    aria-pressed={!!on}
    aria-label={label || (on ? 'Снять отметку' : 'Отметить')}
    onClick={() => onChange?.(!on)}
  />
);
...
export const Tabs = ({ value, onChange, items }) => (
  <div className="tabs">
    {items.map(it => (
      <button type="button" key={it.value} className={`tab ${value === it.value ? 'active' : ''}`} onClick={() => onChange(it.value)}>
        {it.label}
      </button>
    ))}
  </div>
...
export const Segmented = ({ value, onChange, items }) => (
  <div className="seg">
    {items.map(it => (
      <button type="button" key={it.value} className={`seg-btn ${value === it.value ? 'active' : ''}`} onClick={() => onChange(it.value)}>
        {it.label}
      </button>
    ))}
  </div>
...
export const TabsUnderline = ({ value, onChange, items }) => (
  <div className="tabs-underline">
    {items.map(it => (
      <button type="button" key={it.value} className={`tab-u ${value === it.value ? 'active' : ''}`} onClick={() => onChange(it.value)}>
...
export const Metric = ({ label, value, unit, delta, deltaKind, sparkline, hint }) => {
  ...
  return (
    <div className="metric">
      ...
      <div className="metric-value tabular"><NumberPopIn value={value} />{unit && <span className="unit">{unit}</span>}</div>
      {delta != null && (
...
export const Spark = ({ values, color, height = 28, fill = true }) => {
  ...
  return (
    <div className="spark-chart" style={{ height, color: c }}>
  ...
  return (
    <div className="spark-tooltip">
      <div className="chart-tooltip-row">
        <span>Значение</span>
        <strong className="tabular">{payload[0].value}</strong>
      </div>
    </div>
  );
}
