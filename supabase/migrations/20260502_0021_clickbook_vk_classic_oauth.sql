-- ClickBook VK classic OAuth auth
-- Keeps VK accounts in a separate table. user_id is intentionally not a FK to
-- auth.users because ClickBook may use deterministic virtual app-session users
-- for Telegram/VK/social auth.
...
-- Drop possible old FK if any previous experimental table referenced auth.users.
do $$
declare
